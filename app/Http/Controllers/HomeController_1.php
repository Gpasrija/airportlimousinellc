    ...
    public function bookingForm(Request $request){
        ...
        try {
            $response = Mail::to($bookingData['email'])
                ->cc(['info@airportlimoindy.com', 'gourav.luv21@gmail.com', 'pardeepbasson@gmail.com'])
                ->send(new BookingEmail($bookingData));

            if (Mail::failures()) {
                Log::error('Email sending failed for: ' . implode(', ', Mail::failures()));
                session()->flash('message', 'email id is not valid.');
            ...
            session()->flash('message', 'Sorry, Please verify the email id again.');
            session()->flash('alert-type', 'error');    
            return redirect()->route('home');
        ...
        // Return a success response if email sent successfully
